...
export default async function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (event: SessionStartEvent, ctx: ExtensionContext) => {
		...
		server = createServer((req, res) => {
			handleHttpRequest(req, res as ServerResponse, config, ctx, pi, () => isStreaming);
		});
		...
		server.listen(port, host, () => {
			const tokenInfo = config.token ? " (token-protected)" : " (no authentication!)";
			ctx.ui.notify(`Remote server started on http://${host}:${port}${tokenInfo}`, "warning");
...
function handleHttpRequest(
	...
	if (config.token) {
		...
		const url = new URL(req.url || "", `http://${req.headers.host || "localhost"}`);
		const providedToken = url.searchParams.get("token") || authHeader.replace("Bearer ", "");
...
async function handlePostMessage(req: IncomingMessage, res: ServerResponse, ctx: ExtensionContext, pi: ExtensionAPI, isStreaming: boolean): Promise<void> {
	...
	req.on("end", async () => {
		try {
			...
			ctx.ui.notify(`Received remote message: ${message.slice(0, 50)}...`, "info");
			pi.sendUserMessage(message, { deliverAs: isStreaming ? "steer" : undefined });
