...
describe('real server + real pi harness + fake LLM', () => {
  ...
  it('reproduces "pi stops midway": upstream cuts the SSE stream mid-response', async () => {
    ...
    // upstream died mid-stream. If pi truly "stops midway" with no terminal
    // event, this wait times out — which is exactly the bug surfaced as a
    // failing test. Documenting current behavior is step one of TDD-ing the fix.
    const settled = await c.waitFor(
      (m) => m.type === 'agent_end' || m.type === 'session_error',
      30_000,
    );

    // We received the partial text before the cut.
    expect(c.streamedText().length).toBeGreaterThan(0);
    expect(c.streamedText().length).toBeLessThan('this reply gets cut off here and never finishes'.length);
