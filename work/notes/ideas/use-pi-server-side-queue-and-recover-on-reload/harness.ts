...
// EVIDENCE FILE (not an in-tree test). It lives under work/ideas/use-pi-server-side-queue-and-recover-on-reload/.
// When the rewrite adopts it, move it to server/test/ and set serverEntry to
// '../src/index.ts'. From the evidence dir it resolves the real server entry
// relative to the repo root (overridable via WHEREVER_SERVER_ENTRY).
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const repoRoot = path.resolve(__dirname, '../../..');
const serverEntry =
  process.env.WHEREVER_SERVER_ENTRY ?? path.resolve(repoRoot, 'server/src/index.ts');
...
export interface Harness {
  ...
  /** Open a WS client and run the connect handshake. */
 ...
 */
export async function startHarness(opts?: { initial?: FakeBehavior }): Promise<Harness> {
  ...
  waitFor(pred: (m: ServerMsg) => boolean, timeoutMs = 20_000): Promise<ServerMsg> {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(
        () => reject(new Error(`timeout waiting for message; saw: ${this.messages.map((m) => m.type).join(', ')}`)),
        timeoutMs,
      );
      this.waiters.push({
        pred,
