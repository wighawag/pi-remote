<script lang="ts">
	...
	import {mediaKind} from '$lib/core/media-kind';
	import {
		armTtsGestureUnlock,
		speakUtterance,
		unlockTts,
	...
	let msgList = $derived.by(() =>
		filterMessages(
			$messages,
			{
				hideThinking: $piState.hideThinking,
				hideTools: $piState.hideTools,
			},
			streaming,
		),
	);
